...
class Perioderplallold extends REST_Controller {
	...
	public function datarplold_get()
	{
		...
		$set_rplold_awal 	= $this->rpl_croluar->rplawalold_tid($tid , $rpl_awal);
		...
		if($set_rplold_awal == null)
		{
			...
			$returnold_order	= $this->rpl_croluar->returnold_tid($set_rplold_awal->id_order);
			...
			$this->set_response([
				...
				//ambil returnnya
				$rpl_awal 	= $this->rpl_croluar->rpl_oldest($tid, $returnold_order->tglrpl);
				...
				if($rpl_awal == null){
					...
					// $teknisi_flm =  $this->flm_croluar->flm_teknisi_old($tid, $temp_date, $tgl2);
					...
					// $teknisi_flm =  $this->flm_croluar->flm_teknisi_old($tid, $temp_date, $returnold_order->tglrpl);
...
/* End of file Periodeallold.php */
/* Location: ./application/controllers/api/Periodeallold.php */